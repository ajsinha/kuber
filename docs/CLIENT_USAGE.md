...
| `field=[v1\|v2\|v3]` | IN clause (v1.7.8) | `status=[active\|pending]` |
...
# IN clause - match multiple values (v1.7.8)
results = client.json_search('status=[active|pending]')

# Multiple attributes with IN clauses (v1.7.8)
results = client.json_search('status=[active|pending],country=[USA|UK|CA]')

# Using convenience method with dict (v1.7.8)
conditions = {
...
// IN clause - multiple values (v1.7.8)
...
// Using convenience method with Dictionary (v1.7.8)
