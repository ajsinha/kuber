...
| `field=[v1\|v2\|v3]` | IN clause (v1.9.0) | `status=[active\|pending]` |
...
# IN clause - match multiple values (v1.9.0)
results = client.json_search('status=[active|pending]')

# Multiple attributes with IN clauses (v1.9.0)
results = client.json_search('status=[active|pending],country=[USA|UK|CA]')

# Using convenience method with dict (v1.9.0)
conditions = {
...
// IN clause - multiple values (v1.9.0)
...
// Using convenience method with Dictionary (v1.9.0)
